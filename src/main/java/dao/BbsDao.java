...
public class BbsDao {
	...
	public static BbsDao getInstence() {
		if (dao == null) {
			dao = new BbsDao();
		}
	...
	*/
	...
	public List<BbsDto> getBbsPageList(String choice, String search, int pageNumber) {
		String sql = " select seq, id, ref, step, depth, title, content, wdate, del, readcount "
				+ " from (select row_number()over(order by ref desc, step asc) "
				+ " as rnum, seq, id, ref, step, depth, title, content, wdate, del, readcount "
				+ " from bbs ";
		...
		sql += " order by ref desc, step asc) a " + " where rnum between ? and ? ";
		...
		// pageNumber(0, 1, 2...)
		int start, end;
		start = 1 + 10 * pageNumber; // 1 11 21
		end = 10 + 10 * pageNumber; // 10 20 30
	...
	public boolean answer(int seq, BbsDto dto) {
		...
		String sql2 = " insert into bbs(id, ref, step, depth, title, content, wdate, del, readcount) "
				+ " values(?, (select ref from bbs a where seq=?), "
		...
		try {
			...
			// commit 비활성화, commit을 하면 rollback 불가, update로 해야 함 (commit: 적용, 확정 / rollback: 되돌리기)
